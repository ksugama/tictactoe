package keeper_test

import (
	"testing"

	sdk "github.com/cosmos/cosmos-sdk/types"
	"github.com/stretchr/testify/require"
	"google.golang.org/grpc/codes"
	"google.golang.org/grpc/status"

	keepertest "github.com/ksugama/tictactoe/testutil/keeper"
	"github.com/ksugama/tictactoe/testutil/nullify"
	"github.com/ksugama/tictactoe/x/tictactoe/types"
)

func TestGameIdQuery(t *testing.T) {
	keeper, ctx := keepertest.TictactoeKeeper(t)
	wctx := sdk.WrapSDKContext(ctx)
	item := createTestGameId(keeper, ctx)
	for _, tc := range []struct {
		desc     string
		request  *types.QueryGetGameIdRequest
		response *types.QueryGetGameIdResponse
		err      error
	}{
		{
			desc:     "First",
			request:  &types.QueryGetGameIdRequest{},
			response: &types.QueryGetGameIdResponse{GameId: item},
		},
		{
			desc: "InvalidRequest",
			err:  status.Error(codes.InvalidArgument, "invalid request"),
		},
	} {
		t.Run(tc.desc, func(t *testing.T) {
			response, err := keeper.GameId(wctx, tc.request)
			if tc.err != nil {
				require.ErrorIs(t, err, tc.err)
			} else {
				require.NoError(t, err)
				require.Equal(t,
					nullify.Fill(tc.response),
					nullify.Fill(response),
				)
			}
		})
	}
}
